...
while not pr.window_should_close():
    ...
    if key == 51:
        i += 1
        sides = []
        new_line = [Vector(mouse_pos[0], mouse_pos[1]),
                    Vector(mouse_pos[0]+40, mouse_pos[1]+0)]
        ...
        Physics.poly_rigidBody(
            'poly' + str(i), mouse_pos[0], mouse_pos[1], 30, sides)
    ...
    if key == 67:
        i += 1
        Physics.circle_rigidbody(
            'circle' + str(i), mouse_pos[0], mouse_pos[1], 30, 100)
        ...
        if (Physics.rigidBodies[rb].type == 'Poly'):
            for line in Physics.rigidBodies[rb].sides:
                pr.draw_line(int(line[0].x), int(line[0].y), int(
                    line[1].x), int(line[1].y), pr.BLACK)

        elif (Physics.rigidBodies[rb].type == 'Circle'):
            m = Physics.rigidBodies

            pr.draw_circle_lines(int(m[rb].position.x), int(
                m[rb].position.y), m[rb].r, pr.BLACK)
